defmodule Core.GeneratingSeedsTest do
  @moduledoc false
  ...
  test "block was recalculated, but is still detected", %{initial_hash: first_hash} do
    ...
    new_hash =
      BlockAPI.calculated_hash(block_under_test.version, block_under_test.block_start, block_under_test.block_end)

    {:ok, _} = Core.BlockRepo.update(Ecto.Changeset.change(block_under_test, %{hash: new_hash}))
  ...
  test "each block is verified using its own algorithm's version", %{initial_hash: first_hash} do
    ...
    block_versions = Application.get_env(:core, :block_versions)
    Application.put_env(:core, :block_versions, Map.put_new(block_versions, "v3", new_query))
    Application.put_env(:core, :current_block_version, "v3")

    on_exit(fn ->
      Application.put_env(:core, :current_block_version, "v2")
    end)
