defmodule OPS.GeneratingSeedsTest do
  ...
  test "start init genserver" do
    {:ok, %{hash: first_hash}} = BlockAPI.close_day(~D[2014-01-01])
    ...
    {:ok, %{hash: second_hash}} = BlockAPI.close_day(~D[2014-01-02])
    ...
    {:ok, %{hash: _third_hash}} = BlockAPI.close_day(~D[2014-01-03])
