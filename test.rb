...
DAYS.times do |day|
  ...
  seed = conn_seeds.exec("INSERT INTO seeds (hash, debug, inserted_at) VALUES (digest('#{new_seed}', 'sha512'), '#{new_value}', '#{today} 23:59:59') returning hash")[0]['hash']
...
conn.exec("SELECT DISTINCT date(inserted_at) AS day FROM declarations ORDER BY day;").each do |row|
  day = row['day']
  ...
  new_seed = conn.exec("
    WITH concat AS (
      SELECT
        ...
        ), '') AS value FROM declarations WHERE DATE(inserted_at) = '#{day}'
    )
    SELECT digest(value, 'sha512') as new_seed, value FROM concat;
  ")[0]
  ...
  existing_hash = conn_seeds.exec("SELECT hash FROM seeds WHERE date(inserted_at) = '#{day}'").map { |row| row["hash"] }[0]

  if new_hash == existing_hash
    puts "Day #{day} vas verified. It's correct!"
  else
    puts "Day #{day} vas verified. It's not correct!"
